...
const loginController = async (req, res) => {
  try {
    ...
    if (!ismatch) {
      return res
        .status(200)
        .send({ message: "Inavalid Email or Password", success: false });
    }
...
//Delete Notification Controlller
...
const getAllDoctorsController = async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      error,
      message: "Errror While Fetching Docotr",
    });
  }
};
...
const userAppointmentsController = async (req, res) => {
  try {
    ...
    return res.status(200).send({
      success: true,
      message: "Usr Appointments fetched successfully",
      data: appointments,
    });
  } catch (error) {
    console.log(error);
